...
| Semantic circuit and neutral-stone colors | Same indexed world | Two 4× normal + shadowless pairs | Not measured | Same scan | Exact block-level diagnostics exposed `redstone_wire` accidentally matching the substring `stone`, plus missing circuit, lit-pumpkin, and obsidian styles. The redstone-array tile's mean perceptual error fell from 5.490 to 0.360 and its fraction within error 3 rose from 84.2% to 99.1%. Lit pumpkins, obsidian, circuits, and natural stone now match the lossless oracle exactly. |
...
| Directional 256-block shadow input halo | 38.63 s | 3.18 s mean for one normal plus one shadowless 4× tile | Not measured | 1.38 GiB | Decode the northwest shadow halo once, expose it through tile-relative surface lookup, and keep the one-block relief halo on every side. The Stratos 2×2 A/B at X/Z `-2944/-3072` removed both tile-wide brightness steps. Six exact Playwright strips now cover X and Z seams at native zooms 1–3. Four sequential diagnostic pairs took 12.74 s, so cached shadow-map inputs remain an important production optimization. |
...
The committed ten-scene lossless suite also checks snow, coast, dense canopy,
